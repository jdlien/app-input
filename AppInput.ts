...
export default class AppInput {
  ...
  prefixSuffixColor: string = 'bg-zinc-200 text-zinc-600 dark:bg-zinc-700 dark:text-zinc-400'
  prefixSuffixStone: string = 'bg-stone-200 text-stone-600 dark:bg-stone-700 dark:text-stone-400'
  ...
  errorClassDefault: string = 'error hidden text-sm text-red-600 transition dark:text-red-500'
  descriptionClassDefault: string = 'mt-1 text-sm text-zinc-500 dark:text-zinc-400'
  ...
  constructor(attributes: IAttributes & AnyAttributes = {}) {
    ...
    this.prefixClassDefault += ' ' + this.prefixSuffixColor + ' ' + this.borderColor
    this.suffixClassDefault += ' ' + this.prefixSuffixColor + ' ' + this.borderColor
    ...
    this.hasEndTag = ['select', 'textarea', 'button'].some((el) => el === this.type.toLowerCase())
  ...
  generateId(attributes: IAttributes): string {
    ...
    if (attributes.name) return attributes.name + '-' + Math.random().toString(36).slice(2)
  ...
  setupInputAttributes(attributes: IAttributes): IAttributes {
    ...
    switch (attributes.type) {
      case 'select':
        ...
        if (attributes.placeholder) attributes['data-placeholder'] = attributes.placeholder
        break
  ...
  generateAttributesString(attributes: IAttributes): string {
    ...
    const booleanAttributes = ['checked', 'disabled', 'readonly', 'required', 'multiple']
    ...
    if (attributes.maxLength) attributesString += ` data-max-length="${attributes.maxLength}"`
    ...
    if (!this.hasEndTag && attributes.value !== undefined) {
      const escapedValue = String(attributes.value).replace(/"/g, '&quot;').replace(/'/g, '&#39;')
  ...
  normalizeOptions(options: OptionType[]): NormalizedOptionType[] {
    return options.map((opt: any) => {
      ...
      if (opt === undefined || opt.value === undefined) {
        throw new Error('Option must have a value. Remove undefined/NULL values.')
      }

      // Remove any trailing commas from values
      const value = opt.value.length > 1 ? opt.value.replace(/,$/, '') : opt.value
      ...
      if (opt.description !== undefined) normalizedOption.description = opt.description
  ...
  generateOptions(options: NormalizedOptionType[]): string {
    ...
    for (let option of options as NormalizedOptionType[]) {
      ...
      if (option.description) html += `<div class="mt-2 text-sm">${option.description}</div>`
      // End the radio div
      html += `</div>`
    }
  ...
  start(): string {
    ...
    if (['checkbox', 'radio'].includes(this.type) && (!this.options || this.options.length == 0)) {
    ...
    else if (this.type === 'display') html += `<span ${this.attributesString}>${this.value}</span>`
    else html += `${this.prefixEl}<input ${this.attributesString} />${this.colorEl}${this.suffixEl}`
  ...
  // Add the end tag
    ...
    if (this.type === 'select') {
      ...
      const valueArr = typeof this.value !== 'undefined' ? String(this.value).split(',') : []
